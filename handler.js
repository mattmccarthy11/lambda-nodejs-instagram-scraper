  ...
  https.get(`https://www.instagram.com/${username}/`, (resp) => {
    ...
    resp.on('end', () => {
      if(200===resp.statusCode){
        ...
        if(userData && userData.media_next_page){
  ...
  https.get(options, (resp) => {
    ...
    resp.on('end', () => {
      if(200===resp.statusCode){
        ...
        const {image_count, image_engagemets, video_count, video_engagemets, brand_post_engagements, brand_video_engagements} = parseStatFromMedias(medias);
        ...
        userData.brand_post_engagements = userData.brand_post_engagements.concat(brand_post_engagements).sort((a,b)=>a-b).slice(0,10);
        userData.brand_video_engagements = userData.brand_video_engagements.concat(brand_video_engagements).sort((a,b)=>a-b).slice(0,10);
...
const parseStatFromMedias = medias => {
  let stats = {
    ...
    video_count: 0,
    video_engagemets: 0,
    brand_post_engagements: medias.brand_post_engagements || [],
    brand_video_engagements: medias.brand_video_engagements || []
  };
  medias.forEach( post => {
    ...
    if(post.is_video){
      ...
      stats.brand_video_engagements.push(eng + post.view_count);
    }else{
      stats.image_count++;
      stats.image_engagemets += eng;
      stats.brand_post_engagements.push(eng);
    }
  });
  return stats;
};
